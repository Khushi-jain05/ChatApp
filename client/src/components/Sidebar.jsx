...
const Sidebar = () => {
  const {getUsers,users,selectedUser,setSelectedUser,unseenMessages,setUnseenMessages}=useContext(ChatContext)
    ...
    const filterUsers= input? users.filter((user)=>user.fullName.toLowerCase().include(input.toLowerCase())) :users
    useEffect(()=>{
      getUsers()
  ...
  return (
    ...
    <div className='bg-[#282142] rounded-full flex item-center gap-2 py-3 px-4 mt-5'>
        ...
        outline-none text-white text-xs placeholder-[#c8c8c8] flex-1' placeholder='Search User...' />
    </div>
      </div>
      <div className='flex flex-col'>
{filterUsers.map((user,index)=>(
    <div onClick={()=>setSelectedUser(user)}
     key={index} className={`relative flex item-center gap-2 p-2 pl-4 rounded
...
flex justify-center item-center rounded-full bg-violet-500/50'>{unseenMessages[user._id]}</p>}
</div>))}
      </div>
    </div>
  )
